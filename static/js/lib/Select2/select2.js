            ...
            opts = $.extend({}, {
                populateResults: function(container, results, query) {
                    ...
                    populate = function(results, container, depth) {
                        var i, l, result, selectable, compound, node, label, innerContainer, formatted, html;
                        ...
                        for (i = results.length - 1; i >= 0; i = i - 1) {
                            ...
                            node.className = "select2-results-dept-" + depth + " select2-result " + (selectable ? "select2-result-selectable" : "select2-result-unselectable");
                            if (compound) { 
                                node.className += " select2-result-with-children";
                            }
                            ...
                            formatted = opts.formatResult(result, label, query, _cache, oFragement);

                            if (formatted !== undefined) {
                                label.innerHTML = self.opts.escapeMarkup(formatted);
                            }
                            ...
                            oFragement.appendChild(node);
                        }
                        container[0].appendChild(oFragement);
                    };
    ...
    SingleSelect2 = clazz(AbstractSelect2, {
        ...
        createContainer: function() {
            var container = $("<div></div>", {
                ...
                "   </div>",
                "   <ul class='select2-results scroll'>",
                "   </ul>",
                "</div>"
            ].join(""));
            return container;
    ...
    MultiSelect2 = clazz(AbstractSelect2, {
        ...
        addSelectedChoice: function(data) {
            ...
            $(data).each(function(i, d) {
                var id = self.id(d),
                    formatted = self.opts.formatSelection(d);
                // select2-data has some problem to insert
                choiceTpl += [
                    '<li class="select2-search-choice">',
                    '<div>', self.opts.escapeMarkup(formatted), '</div>',
            ...
            var $choice = $(choiceTpl);
            $choice.each(function(i, e) {
                $(e).data("select2-data", data[i]);
            });
            $choice.find(".select2-search-choice-close")
                .bind("mousedown", killEvent)
                .bind("click dblclick", self.bind(function(e) {
                    if (!self.enabled) return;

                    $(e.target).closest(".select2-search-choice").fadeOut('fast', self.bind(function() {
