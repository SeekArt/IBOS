...
Ibos.core = {
    ...
    inherits: function(_subClass, _superClass) {
        ...
        _subClass.prototype = $.extend(new _F(), {
            _super: _superClass,
            constructor: _subClass
        }, _subClass.prototype);
        _F.prototype = null;
        return _subClass;
    }
        ...
        PseudoSelect.prototype = {
            ...
            _setSelectedValue: function(value, text) {
                ...
                if (this.options.template) {
                    tpl = $.template(this.options.template, {
                        text: text
                    });
                } else {
                    tpl = text;
        ...
        Ui.PopMenu = function($ctrl, $menu, options) {
            ...
            var _hide = function(e) {
                ...
                if (that.options.trigger === "hover") {
                    if (e.toElement === $ctrl[0] || e.toElement === $menu[0] || $.contains($ctrl[0], e.toElement) || $.contains($menu[0], e.toElement)) {
                        return false;
                    }
                }
                timer = setTimeout(function() {
...
$.fn.waiting = function(content, mode, lock) {
    ...
    var modeSet = {
        ...
        small: $.template(tpl, {
            path: "/image/loading_small.gif",
            size: 24,
            fontSize: 14,
            content: content
        }),
        mini: $.template(tpl, {
            path: "/image/loading_mini.gif",
            size: 16,
            fontSize: 12,
            content: content
        })
...
Ibos.CmList.prototype = {
    ...
    removeItem: function(id) {
        ...
        var _remove = function() {
            $(_this).trigger("beforeitemremove", {
                item: $item
            });
            $item.remove();
            $(_this).trigger("itemremove", {
                item: $item
            });
        }
        if (this.opts.animate) {
...
(function() {
    ...
    $(document.body).on('keydown', function(evt) {
        //获取事件对象
        var evt, elem, code, _todo;

        evt = evt || window.event;
        elem = evt.relatedTarget || evt.srcElement || evt.target || evt.currentTarget;
        code = evt.keyCode;

        if (code in KEYBOARD) {
            _todo = '_' + KEYBOARD[code].toLowerCase();
            _fn[_todo].call(null, elem, evt);
        }
    });
    ...
    _fn._backspace = function(elem, evt) {
        ...
        if (nodeName == 'input' && (type != 'text' && type != 'textarea' && type != 'password' && type != 'file')) {
            return _kill(evt);
        }

        if (nodeName == 'input' && (elem.readOnly == true || elem.disabled == true)) {
            return _kill(evt);
        }
    };

    _fn._enter = function(elem, evt) {
        var forms = document.forms,
            $form = $(forms[0]),
            nodeName = elem.nodeName ? elem.nodeName.toLowerCase() : '',
            len, val, submitAuto = true,
        ...
        // 表单字段数大于4时
        // 存在非空输入则不允许回车提交
        if ((len = $inputs.length) > 4) {
            for (len; len--;) {
                val = $inputs[len];
                val === '' && (submitAuto = false);
            }
