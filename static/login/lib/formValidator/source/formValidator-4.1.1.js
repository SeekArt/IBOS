...
;(function($) {
	...
	$.formValidator = 
	{
		...
		initConfig : function(controlOptions)
		{
			...
			//如果填写了表单和按钮，就注册验证事件
			if(settings.formID !== ""){
				$("#"+settings.formID).bind("submit.formValidator", function(){
		...
		{      
			...
			else
			{
				...
				else
				{
					...
					{
						jqobj = $(elem);
						var allClass=onShowClass+" "+onFocusClass+" "+onCorrectClass+" "+onErrorClass; //添加此条为不破坏表单原有样式 @by Aeolus 2011年7月29日
						if(onShowClass!="" && showclass == "onShow"){
							jqobj.removeClass(allClass).addClass(onShowClass)
						};
						if(onFocusClass!="" && showclass == "onFocus"){
							jqobj.removeClass(allClass).addClass(onFocusClass)
						};
						if(onCorrectClass!="" && showclass == "onCorrect"){
							jqobj.removeClass(allClass).addClass(onCorrectClass)
						};
						if(onErrorClass!="" && showclass == "onError"){
							jqobj.removeClass(allClass).addClass(onErrorClass)
						};
					}
				}
			}
			...
			{		
				...
				else
				{
					...
					errorTimer = setTimeout(function(){
						$.formValidator.setTipState(elem,showclass, null);
					}, 1500);
				}
			}
			else
			...
			{	
				...
				success : function(data, textStatus, jqXHR){
					...
					if((typeof lb_ret)=="string")
					{
						...
						setting.onError = lb_ret; 
					//这句是新增的，错误提示是根据onError的值，当lb_ret是字符串时，onError应该等于lb_ret
					//注：控件值没改变再次失去焦点时是不会再提交验证的，直接会提示onError的值
					//@denglh 2012/4/17
					}
					else if(lb_ret){
						lb_isValid = true;
					...
					//alert(ls_msg) @denglh 屏蔽，lb_ret等于假的时候就弹窗，一般情况不需要这个大的动作,需要alert直接在自定义的success处理
					}
		...
		{   
			...
			{	
				...
				{   
					...
					if($("#"+tip).get(0)==undefined){
						var formValidateTip = $("<div style='position:absolute; z-index:10000' id='"+tip+"'></div>").appendTo($("body")); 
					}else{
						var formValidateTip = $("#"+tip);
					}
          ...
          */
				if($("#"+tip)!=undefined) var formValidateTip = $("#"+tip);
				formValidateTip.css("display","none");
			}
			...
			if(fixTip.length==1 && onMouseOutFixTextHtml!="" && onMouseOnFixTextHtml!="" && showFixText != "")
			{
				...
				fixTip.css("padding","0").css("margin","0").html(onMouseOutFixTextHtml.replace(/\$data\$/g, setting.onShowFixText)); 
			}
			...
			if(srcTag == "input" || srcTag=="textarea")
			{
				...
				// 设置tip定位
				var _tip = $("#" + setting.tipID);
				var _pos = function(){
					var _rel;
					if(!setting.relativeID){
						var relativeID = setting_temp.relativeID ? setting_temp.relativeID : id;
						setting.relativeID = relativeID;
					}
					_rel = $("#" + setting.relativeID);
					var  o = setting.tipCss; //o为默认定位
					var offset =  _rel.offset();

					setting_temp.tipCss.top = offset.top + o.top - o.height; 
					setting_temp.tipCss.left = offset.left +  _rel.outerWidth() + o.left - o.width; 
					_tip.css(setting_temp.tipCss);
				}
				_pos();
				...
				//按键时候触发校验
				jqobj.bind("keyup.formValidator",function(){
					if(this.validatorPasswordIndex > 0)
					{
						try{
				...
				jqobj.bind(setting.triggerEvent + ".formValidator", function(){
					...
					else
					{
						...
						if(!returnObj.isValid)
						{  
							showmsg = $.formValidator.showMessage(returnObj);
							//自动修正错误
			...
			else if (srcTag == "select")
			{
				jqobj.bind({
					//获得焦点
					"focus.formValidator": function(){	
						if (initConfig.mode != "AlertTip") {
							$.formValidator.setTipState(this, "onFocus", setting.onFocus)
						};
					},
					//失去焦点
					"blur.formValidator": function(){
						if(this.validValueOld==undefined || this.validValueOld==jqobj.val()){
							$(this).trigger("change")
						}
					},
					//选择项目后触发
					"change.formValidator": function(){
						var returnObj = $.formValidator.oneIsValid(id);	
						if(returnObj==null){
							return;
						}
	...
	$.fn.showTooltips = function()
	{
		if($("body [id=fvtt]").length==0){
			fvtt = $("<div id='fvtt' style='position:absolute;z-index:56002; color:#FC7D8E; padding:0 3px; font-size:12px; border:1px dashed #FC7D8E; border-radius:3px; '></div>");
			$("body").append(fvtt);
		...
		return this.each(function()
		{
			...
			//注册事件
			jqobj.bind({
				"mouseover.formValidator" : function(e){
					$("#fvtt").empty().append(this.tooltip).show();
					$("#fv_content").html(this.Tooltip);
					$.formValidator.localTooltip(e);
				},
				"mouseout.formValidator" : function(){
					$("#fvtt").hide();
				},
				"mousemove.formValidator": function(e){
					$.formValidator.localTooltip(e);
				}
			});
		});
