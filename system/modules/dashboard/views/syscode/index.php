...
<div class="ct">
	...
	<div>
		<form id="sys_code_form" action="<?php echo $this->createUrl( 'syscode/index' ); ?>" class="form-horizontal" method='post'>
			<!-- 系统代码设置 start -->
			<div class="ctb">
					...
					<table class="mst table table-bordered table-striped table-operate" id="system_code_table">
						...
						<?php foreach ( $data as $key => $val ): ?>
							...
							<tbody class="mstp <?php if ( $count == 1 ): ?>active<?php endif; ?>">
								<tr>
									<td class="mst-icon"><a href="javascript:void(0);" data-target="#system_code_<?php echo $key; ?>_body" data-act="collapse"></a></td>
									<td>
										<input type="text" class="input-small" name='codes[<?php echo $key; ?>][sort]' value="<?php echo $val['sort']; ?>">
									</td>
									<td>
										<div class="row">
											<div class="span6">
												<input type="text" class="input-small" data-type="name" name='codes[<?php echo $key; ?>][name]' value="<?php echo $val['name']; ?>">
											</div>
											<div class="span6">
												<a href="javascript:void(0);" data-target="#system_code_<?php echo $key; ?>_body" class="cbtn o-plus" data-act="add"></a>
											</div>
										</div>
									</td>
									<td>
							...
							<tbody class="msts" data-id='<?php echo $key; ?>' id="system_code_<?php echo $key; ?>_body" 
								   ...
								   <?php foreach ( $val['child'] as $subKey => $subVal ): ?>
									   <?php $subIndex++; ?>
									<tr data-id='<?php echo $subVal['id']; ?>' <?php if ( $subIndex == $subCount ): ?>class="msts-last"<?php endif; ?>>
										<td></td>
										<td>
											<input type="text" name='codes[<?php echo $subVal['id']; ?>][sort]' class="input-small" value="<?php echo $subVal['sort']; ?>">
										</td>
										<td class="mst-board">
											<div class="row">
												<div class="span2"></div>
												<div class="span6">
													<input type="text" name='codes[<?php echo $subVal['id']; ?>][name]' data-type="name" class="input-small" value="<?php echo $subVal['name']; ?>">
												</div>
												<?php if ( $subVal['system'] == '0' ): ?>
													<div class="span4"><a href="javascript:void(0);" data-target="<?php echo $subVal['id']; ?>" data-act="remove" class="cbtn o-trash"></a></div>
												<?php endif; ?>
											</div>
										</td>
										<td>
											<?php if ( $subVal['system'] == '0' ): ?>
												<input type="text" name='codes[<?php echo $subVal['id']; ?>][number]' data-type="number" class="input-small" value="<?php echo $subVal['number']; ?>">
											<?php else: ?>
				...
				<div>
					<button type="submit" name='sysCodeSubmit' class="btn btn-primary btn-large btn-submit"><?php echo $lang['Submit']; ?></button>
				</div>
			</div>
