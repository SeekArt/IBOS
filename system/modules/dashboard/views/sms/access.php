...
<div class="ct">
	...
	<div>
		<form onsubmit="return beforeSubmit()" action="<?php echo $this->createUrl( 'sms/access' ); ?>" method="post" class="form-horizontal">
			<!-- 指定模块权限 start -->
			<div class="ctb">
				...
				<div class="ctbw cross-selector">
					<div class="row">
							...
							<select id="select_left" multiple>
								...
								<?php foreach ( $enableModule as $module ): ?>
									<?php if ( in_array( $module['module'], $smsModule ) ): ?>
										<option value="<?php echo $module['module'] ?>"><?php echo $module['name'] ?></option>
									<?php else: ?>
										<?php $disabledModule[] = $module; ?>
									<?php endif; ?>
								<?php endforeach; ?>
							</select>
							<button type="button" class="btn" id="select_all_left"><?php echo $lang['Select all']; ?></button>
						</div>
						<div class="span2">
						...
						<div class="span5">
							...
							<button type="button" class="btn" id="select_all_right"><?php echo $lang['Select all']; ?></button>
						</div>
					</div>
				</div>
			</div>
			<div>
				...
				<button name="smsSubmit" class="btn btn-primary btn-large btn-submit" type="submit"><?php echo $lang['Submit']; ?></button>
			</div>
		</form>
	</div>
</div>
