<div class="limit-edit-dialog" id="limit_edit_dialog">
	<form class="form-horizontal" method="post" id="limit_edit_from">
		<div class="control-group">
			...
			<div class="controls">
				<input type="hidden" id="role_select" name="roleid" value="<?php echo $roleid; ?>" placeholder="请选择角色"/>
			</div>
		</div>
			...
			<div class="dsh-limit-box">
				<div class="dsh-limit-body fill">
					...
					<ul class="dsh-limit-list clearfix">
						...
						<?php $isData = $node['type'] === 'data';$checked = isset( $nodeRelated[$node['module']][$node['key']] ); ?>
						<?php if($node['type'] == "data"):?>
						<li class="dsh-limit-privilege-wrap">
							<label <?php if ( $checked ): ?>class="active"<?php endif; ?>>
								<input type="checkbox" <?php if ( $checked ): ?>checked<?php endif; ?> name="nodes[<?php echo $node['id']; ?>]" value="<?php echo $isData ? 'data' : $node['id']; ?>" data-node="funcCheckbox" data-pid="<?php echo $mkey;?>">
								<?php if ( $isData ): ?>
								<div class="dsh-limit-privilege">
									<?php foreach($node['node'] as $data):?>
									<?php $checked = isset( $nodeRelated[$data['module']][$data['key']][$data['node']] ); ?>
									<input value="<?php echo $checked ? $nodeRelated[$data['module']][$data['key']][$data['node']] : ''; ?>" <?php if($checked): ?>checked<?php endif; ?> name="data-privilege[<?php echo $node['id']; ?>][<?php echo $data['id']; ?>]" type="text" data-text="<?php echo $data['name'];?>" data-toggle="privilegeLevel" style="display: none;">
									<?php endforeach;?>
								</div>
								<?php endif; ?>
						...
						<li>
							<label <?php if ( $checked ): ?>class="active"<?php endif; ?>>
								<input type="checkbox" <?php if ( $checked ): ?>checked<?php endif; ?> name="nodes[<?php echo $node['id'];?>]" value="<?php echo $node['id'];?>" data-node="funcCheckbox" data-pid="<?php echo $mkey;?>">
