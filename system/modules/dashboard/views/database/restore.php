		...
		<div class="ctb">
			...
			<div class="page-list">
				<div class="page-list-header">
					<div class="row">	
						<div class="span8">
							<button type="button" data-act="del" class="btn"><?php echo $lang['Delete backup']; ?></button>
						</div>
					</div>
				</div>
				<div class="page-list-mainer">
					<form id="sys_dbrestore_form" method="post" class="form-horizontal" action="<?php echo $this->createUrl( 'database/restore' ); ?>">
							...
							<?php foreach ( $list['exportLog'] as $key => $value ) : ?>
								...
								<tbody>
									<tr>
										<td>
											<label class="checkbox">
												<input type="checkbox" data-check='key' value="<?php echo $key; ?>"  name="key[<?php echo $key; ?>]">
											</label>
										</td>
										<td>
											<a href="#restore_<?php echo $random; ?>_body" data-act="collapse"><?php echo $key; ?></a>
										</td>
										...
										<td>
											<a data-act="import" data-id="<?php echo $info['filename']; ?>" href="javascript:void(0);" class="btn btn-small"><?php echo $lang['Import']; ?></a>
										</td>
									</tr>
								</tbody>
								<!-- 子列表 -->
									...
									<tr>
										<td>
											<label class="checkbox">
												...
												<input type="checkbox" data-check='key' value="<?php echo basename( $value['filename'] ); ?>"  name="key[<?php echo $key; ?>]">
											</label>
										</td>
										<td>
										...
										<td></td>
										<td>
											<a href="javascript:void(0);" data-act="decompress" data-id="<?php echo $value['filename']; ?>" class="btn btn-small"><?php echo $lang['Decompress']; ?></a>
										</td>
									</tr>
								</tbody>
							<?php endforeach; ?>
	...
	(function() {
		...
		$('[data-act="import"]').on('click', function() {
			...
			var dialog = $.artDialog({
				...
				lock: true,
				content: '<?php echo $lang['Confirm import']; ?>',
		...
		$('[data-act="decompress"]').on('click', function() {
			...
			$.artDialog({
				title: "<?php echo IBOS::lang( 'Confirm action', 'message' ); ?>",
				lock: true,
				content: '<?php echo $lang['Confirm decompress']; ?>',
				id: 'confirm_import_act',
				ok: function() {
					window.location.href = url;
				}
