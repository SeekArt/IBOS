			...
			<div class="ctb">
				...
				<table class="point-table table table-bordered table-striped table-operate" id="point_setup_table">
					...
					<tbody id="point_setup_tbody">
						<?php foreach ( $data as $val ): ?>
							<tr id="<?php echo $val['cid']; ?>">
								...
								<td>
									<input type="text" class="input-small" name="credit[<?php echo $val['cid']; ?>][initial]" value="<?php echo $val['initial']; ?>">
								</td>
								<td>
									<input type="text" class="input-small" name="credit[<?php echo $val['cid']; ?>][lower]" value="<?php echo $val['lower']; ?>">
								</td>
								<td>
									<input type="checkbox" name="credit[<?php echo $val['cid']; ?>][enable]"
										   <?php if ( $val['enable'] == 1 ): ?>checked<?php endif; ?> data-toggle="switch" class="visi-hidden">
								</td>
								<td>
									<?php if ( $val['system'] != 1 ): ?>
					...
					<tfoot>
						<tr>
							<td colspan="4">
								<a href="javascript:;" data-action="addCreditRule" class="cbtn o-plus" title="<?php echo $lang['Add credit']; ?>"></a>
							</td>
							<td>
								<a href="javascript:;" data-action="resetCreditRule" class="cbtn o-cancel" title="<?php echo $lang['Restore system Settings']; ?>"></a>
							</td>
						</tr>
					</tfoot>
				</table>
				<div>
					<div class="control-group">
						...
						<div class="controls">
							<input type="checkbox" name="changeRemind" data-toggle="switch" class="visi-hidden" <?php if ( $changeRemind ): ?>checked<?php endif; ?>>
						</div>
					</div>
				</div>
			</div>
