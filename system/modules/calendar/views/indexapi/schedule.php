		...
		<tbody>
			<?php foreach ( $schedules as $k => $schedule ): ?>
				<tr id="cal_list_<?php echo $k; ?>" class="cal-list-first">
					...
					<td width="90" <?php if($k+1<count($schedules) && date('Y-m-d', $schedule['starttime']) == date('Y-m-d',$schedules[$k+1]['starttime'] ) ): ?>style="border-bottom:none"<?php endif; ?>></td>
					<?php else: ?>
					<td width="90" <?php if($k+1<count($schedules) && date('Y-m-d', $schedule['starttime']) == date('Y-m-d',$schedules[$k+1]['starttime'] ) ): ?>style="border-bottom:none"<?php endif; ?>>
						<div class="mini-date">
							<strong><?php echo $schedule['dateAndWeekDay']['day']; ?></strong>
							<div class="mini-date-body">
								<p><?php echo $schedule['dateAndWeekDay']['weekday']; ?></p>
								<p><?php echo $schedule['dateAndWeekDay']['year']; ?>-<?php echo $schedule['dateAndWeekDay']['month']; ?></p>
							</div>
						</div>
					</td>
					<?php endif; ?>
					<td width="90">
						...
						<?php else: ?>
						<span class="tcm"><?php echo date('H:i', $schedule['starttime']); ?>-<?php echo date('H:i', $schedule['endtime']); ?></span>
						<?php endif; ?>
					</td>
					<td>
						<p class="xcm cal-item-title" title="<?php echo $schedule['subject']; ?>"><?php echo $schedule['cutSubject']; ?></p>
					</td>
				</tr>
			<?php endforeach; ?>
		</tbody>
