    ...
    <div class="sbb sbbl sbbf">
        <ul class="nav nav-strip nav-stacked">
            ...
            <li <?php if ( $this->id == 'review' ): ?>class="active"<?php endif; ?>>
				...
				<!-- 汇报类型 -->
				<?php $typeid = Env::getRequest( 'typeid' ); $getUid = Env::getRequest('uid'); $getUser = Env::getRequest( 'user' ); ?>
				<div class="rp-cycle">
					...
					<ul class="aside-list" id="rp_type_aside_list">
						<?php foreach($reportTypes as $reportType): ?>
							...
							<li <?php if($reportType['typeid'] == $typeid): ?>class="active"<?php endif; ?> data-id="<?php echo $reportType['typeid'] ?>">
									...
									<?php foreach ( $dept['user'] as $user ): ?>
		                                <li>
		                                    <div class="mng-item">
		                                        ...
		                                        <a href="<?php echo $this->createUrl( 'review/index', array( 'op' => 'personal', 'uid' => $user['uid'], 'typeid' => $typeid ) ); ?>" <?php if ( Env::getRequest( 'uid' ) == $user['uid'] ): ?>style="color:#3497DB;"<?php endif; ?>>
