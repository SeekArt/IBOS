					...
					<div class="page-list-mainer">
						<table class="rp-detail-table">
										...
										<div class="fill-ss">
											<div class="pull-right">
												<div class="calendar-group pull-left">
													<div class="datepicker form_datetime" id="date_summary_start" data-value="<?php echo date('Y-m-d', $report['begindate']); ?>">
								...
								<tr>
									...
									<td>
										<ul class="np-list bdbs" id="rp_out_plan_list">
											...
											<li class="np-list-row" data-id="<?php echo $outSidePlan['recordid'] + 100; ?>">
												<div class="bamboo-pgb pull-right">
													<a href="javascript:;" class="o-trash cbtn pull-right ml" title="<?php echo $lang['Delete']; ?>" data-id="<?php echo $outSidePlan['recordid'] + 100; ?>"></a>
													...
													<input  id="processinput_<?php echo $outSidePlan['recordid'] + 100; ?>" type="hidden" name="outSidePlan[<?php echo $outSidePlan['recordid'] + 100; ?>][process]" value="<?php echo $outSidePlan['process']; ?>">
												</div>
												<span class="rp-detail-num" data-toggle="badge"><?php echo count( $orgPlanList ) + $k2 + 1; ?>.</span>
												<input type="text" name="outSidePlan[<?php echo $outSidePlan['recordid'] + 100; ?>][content]" class="rp-input span7" value="<?php echo $outSidePlan['content']; ?>" data-id="<?php echo $outSidePlan['recordid'] + 100; ?>">
											</li>
											<?php endforeach; ?>
										</ul>
										<div class="fill-sn">
													...
													<div class="attl-item" data-node-type="attachItem">
														...
														<span class="fss mlm">
						                                    ...
						                                    <?php if (isset($attach['officeediturl'])): ?>
						                                        <a href="javascript:;" class="mlm" data-action="editOfficeFile" data-param='{"href": "<?php echo $attach['officeediturl']; ?>"}' title="<?php echo $lang['Edit']; ?>">
						                                            <?php echo $lang['Edit']; ?>
						                                        </a>
						                                    <?php endif; ?>
															<!-- 转存到文件柜，等实现文件柜功能再开启 -->
							...
							<tbody id="rp_plan">
								<tr>
									<td colspan="3">
										<div class="fill-ss">
											<div class="pull-right">
												<div class="calendar-group pull-left">
													<div class="datepicker form_datetime" id="date_plan_start" data-value="<?php echo date('Y-m-d', $nextPlanDate['planBegindate']); ?>">
														<a href="javascript:;" class="datepicker-btn" ></a>
												...
												<div class="btn-group ml">
													<button type="button" class="btn" id="date_plan_prev" data-action="prevPlanDate" data-param='{"type": "<?php echo $reportType['intervaltype'];  ?>", "intervals": "<?php //echo $intervals; ?>" }'>
														<i class="glyphicon-chevron-left"></i>
													</button>
													<button type="button" class="btn" id="date_plan_next" data-action="nextPlanDate" data-param='{"type": "<?php echo $reportType['intervaltype'];  ?>", "intervals": "<?php //echo $intervals; ?>" }'>
														<i class="glyphicon-chevron-right"></i>
													</button>
												</div>
											</div>
								...
								<tr>
									<th width="68" class="sep" id="rp_plan_rowspan"><?php echo $lang['Work']; ?><br /><?php echo $lang['Plan']; ?></th>
									<td class="sep" width="3"></td>
									<td>
...
<script type="text/template" id="tpl_rp_new_plan">
	<li class="np-list-row" data-id="<%= id %>">
		...
		<input type="hidden" name="nextPlan[<%= id %>][reminddate]" class="remind-value" value="<%= reminddate %>" nofocus>
		<div class ="rp-vernier-size"></div>
		<div class="posr">
			<div class="pull-right">
				<% if(Ibos.app.g("isInstallCalendar") == 1){ %>
					<div class="rp-remind-bar" <% if(reminddate) { %>style="display: inline-block"<% } %>>
						<i class="o-clock"></i>
						<span class="remind-time"><%= reminddate %></span>
						<a href="javascript:;" class="o-close-small"></a>
					</div>
					<a href="javascript:;" class="co-clock remind-time-btn" title="<%= Ibos.l('RP.SETUP_REMIND') %>" <% if(reminddate) { %>style="display: none;"<% } %>></a>
				<% } %>
				<a href="javascript:;" class="cbtn o-trash mlm" data-id="<%= id %>" title="<%= Ibos.l('DELETE') %>"></a>
			</div>
			<span class="rp-detail-num" data-toggle="badge"><%= index %></span>
			<input type="text" name="nextPlan[<%= id %>][content]" class="rp-input span7" value="<%= content %>" data-id="<%= id %>" placeholder="<%= Ibos.l('RP.CLICK_TO_WRITE_PLAN') %>" >
		</div>
	</li>
</script>
