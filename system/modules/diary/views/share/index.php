			...
			<div class="page-list">
				<div class="page-list-header">
					...
					<div class="btn-group pull-left mls">
						<a href="<?php echo $this->createUrl( 'share/index', array( 'date' => 'yesterday' ) ) ?>" class="btn <?php $date = Env::getRequest( 'date' );
							if ( $date == 'yesterday' || !isset( $date ) ): ?>active<?php endif; ?>"><?php echo $lang['Yesterday']; ?></a>
						<a href="<?php echo $this->createUrl( 'share/index', array( 'date' => 'today' ) ) ?>" class="btn
							<?php if ( Env::getRequest( 'date' ) == 'today' ): ?>active<?php endif; ?>"><?php echo $lang['Today']; ?></a>
					</div>
					<div class="btn-group pull-right">
						<a href="<?php echo $this->createUrl( 'share/index', array( 'date' => $prevAndNextDate['prev'] ) ); ?>" class="btn">
							<i class="glyphicon-chevron-left"></i>
						</a>
						<a <?php if ( TIMESTAMP > $prevAndNextDate['nextTime'] ): ?>
								href="<?php echo $this->createUrl( 'share/index', array( 'date' => $prevAndNextDate['next'] ) ); ?>" class="btn"
							<?php else: ?>
						...
						<li class="da-list-item">
							<div class="da-summary">
								<div class="avatar-box">
									<a href="<?php echo $this->createUrl( 'share/index', array( 'op' => 'personal', 'uid' => $diary['uid'] ) ) ?>" class="avatar-circle">
										<img class="mbm" src="<?php echo $diary['user']['avatar_middle']; ?>" alt="">
									</a>
								...
								<div class="das-content">
									...
									<div class="da-list-item-desc">
										<div class="pull-right">
											<a href="<?php echo $this->createUrl( 'share/show', array( 'diaryid' => $diary['diaryid'] ) ) ?>" target="_blank" class="o-more cbtn" title="<?php echo $lang['More']; ?>"></a>
										</div>
										<span class="fss"><?php echo $diary['addtime']; ?></span>
									</div>
								</div>
							</div>
							<div class="da-detail" data-id="<?php echo $diary['diaryid']; ?>" style="display:none;"></div>
...
<script>
	Ibos.app.setPageParam({
		'commentDisable':  '<?php echo $dashboardConfig['sharecomment']; ?>' != 1
	})
</script>
...
<script>
	$(function() {
		...
		$('#da_date_btn').datepicker().on("changeDate", function(evt) {
			location.href = Ibos.app.url("diary/share/index", { date: $.data(this, "date") });
		});
	});
</script>
