						...
						<?php foreach ( $dept['user'] as $user ): ?>
							<li>
								<div class="mng-item mng-item-user">
									<span class="o-caret" <?php if ( $user['hasSub'] ): ?> data-action="toggleSubUnderlingsList" <?php endif; ?> data-uid="<?php echo $user['uid']; ?>" data-param='{"uid": "<?php echo $user['uid']; ?>"}'><?php if ( $user['hasSub'] ): ?><i class="caret"></i><?php endif; ?></span>
									<a href="<?php echo $this->getController()->createUrl( $userRoute, array( 'uid' => $user['uid'] ) ); ?>" <?php if ( Env::getRequest( 'uid' ) == $user['uid'] ): ?>style="color:#3497DB;"<?php endif; ?>>
										<img src="static.php?type=avatar&uid=<?php echo $user['uid']; ?>&size=middle&engine=<?php echo ENGINE; ?>" alt="<?php echo $user['realname']; ?>">
										<?php echo $user['realname']; ?>
									</a>
									<!-- if 未关注 -->
									<?php if ( Diary::getIsAttention( $user['uid'] ) ): ?>
										<a href="javascript:;" data-node-type="udstar" class="o-gudstar pull-right" data-action="toggleAsteriskUnderling" data-id="<?php echo $user['uid'] ?>" data-param='{"id": "<?php echo $user['uid'] ?>"}'></a> 
									<?php else: ?>
										<a href="javascript:;" data-node-type="udstar" class="o-udstar pull-right" data-action="toggleAsteriskUnderling" data-id="<?php echo $user['uid'] ?>" data-param='{"id": "<?php echo $user['uid'] ?>"}'></a>
									<?php endif; ?>
								</div>
								<!--下属资料,ajax调用生成-->
							</li>
	...
	Ibos.evt.add({
		...
		"toggleSubUnderlingsList": function(param, elem) {
			...
			if (!$elem.attr('data-init')) {
				$.get(Ibos.app.url("diary/review/index", {'op': 'getsubordinates', 'act': "<?php echo $fromController; ?>"}), {uid: param.uid}, function(data) {
