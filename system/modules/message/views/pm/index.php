		...
		<div class="page-list" id="pm_list">
			<div class="page-list-header">
				<div class="pull-left">
					...
					<button type="button" data-action="sendPm" class="btn btn-primary"><?php echo $lang['Send pm']; ?></button>
					<button type="button" class="btn" data-action="removePms"><?php echo $lang['Delete']; ?></button>
					<?php if ( $unreadCount > 0 ): ?><button type="button" class="btn" data-action="markPmsRead"><?php echo $lang['Set read']; ?></button><?php endif; ?>
				</div>
			</div>
			<div class="page-list-mainer">
				<?php if ( $unreadCount > 0 ): ?>
					<div class="band band-primary">
						<?php echo $lang['View'] ?> <strong><?php echo $unreadCount; ?></strong> <?php echo $lang['New conversation']; ?>
						<a href="javascript:;" data-action="markAllPmRead" class="anchor ilsep"><?php echo $lang['Set all read']; ?></a>
					</div>
				<?php endif; ?>
				<?php if ( !empty( $list ) ): ?>
						...
						<?php foreach ( $list as $row ): ?>
							<li class="main-list-item curp" id="pm_<?php echo $row['listid']; ?>" data-url="id=<?php echo $row['listid']; ?>&type=<?php echo $row['type']; ?>">
								<div class="avatar-box pull-left posr">
									...
									<?php else: ?>
									<a href="<?php echo $row['touserinfo'][$row['lastmessage']['touid'][0]]['space_url']; ?>" data-toggle='usercard' data-param='uid=<?php echo $row['touserinfo'][$row['lastmessage']['touid'][0]]['uid']; ?>' class="avatar-circle">
										<img class="mbm" src="<?php echo $row['touserinfo'][$row['lastmessage']['touid'][0]]['avatar_middle']; ?>">
									</a>
									<?php endif; ?>
										...
										<div>
											<div class="pull-left">
												<label class="checkbox checkbox-inline mbz">
													<input type="checkbox" value="<?php echo $row['listid']; ?>" name="pm">
												</label>
												<span class="tcm fss"><?php echo Convert::formatDate( $row['listctime'], 'u' ); ?></span>
												<span class="tcm fss ilsep">|</span>
												<span class="tcm fss"><?php echo $lang['Total']; ?><?php echo $row['messagenum']; ?><?php echo $lang['Item']; ?></span>
											</div>
											<div class="pull-right">
												<a href="javascript:;" title="<?php echo $lang['Delete']; ?>" class="cbtn o-trash" data-action="removePm" data-param='{"id": "<?php echo $row['listid']; ?>"}'></a>
											</div>
										</div>
									</div>
								</div>
