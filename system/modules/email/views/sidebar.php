	...
	<div>
		<div>
			<div class="sbbtw clearfix">
				<!-- 跳转 -->
				<a href="javascript:;" class="pull-right o-mal-setup" title="<?php echo $lang['Setup']; ?>" data-click="setupFolder" data-param="{&quot;url&quot;:&quot;<?php echo $this->createUrl( 'folder/index' ); ?>&quot;}"></a>
			...
			<ul class="sbb-list" data-node-type="folderList">
				<?php foreach ( $folders as $folder ): ?>
					<li data-node-type="folderItem" data-folder-id="<?php echo $folder['fid'] ?>" <?php if ( $fid == $folder['fid'] ): ?>class="active"<?php endif; ?>>
				...
				<select name="search[folder]" id="" class="input-small">
					...
					<?php if ( $hasArchive ): ?>
						<optgroup label="<?php echo $lang['Email archive']; ?>">
							<?php foreach ( $archiveTable['info'] as $tableId => $archiveInfo ): ?>
								<?php if ( $tableId != 0 ): ?>
									<option value="archive_<?php echo $tableId; ?>"><?php echo $archiveInfo['displayname']; ?></option>
								<?php endif; ?>
							<?php endforeach; ?>
						</optgroup>
					<?php endif; ?>
