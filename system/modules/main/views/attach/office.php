									...
									<?php if ( $param['op'] == 'edit' ): ?>
										<div class="mb">
											<div class="mb ovh posr">
												<span class="fun-title"><?php echo $lang['File operations']; //文件操作                          ?></span>
												<span class="fun-divider"></span>
											</div>
											<button type="button" data-action="save" data-param='{"flag":0}' class="btn btn-primary db fun-btn mbs"><?php echo $lang['Save the file']; //保存文件                          ?></button>
											<button type="button" data-action="chgLayout" class="btn fun-btn db mbs"><?php echo $lang['Page setup']; //页面设置                          ?></button>
											<button type="button" data-action="print" class="btn fun-small-btn mbs"><?php echo $lang['Print']; //打印                          ?></button>
											<button type="button" data-action="export" class="btn fun-small-btn mbs"><?php echo $lang['Export pdf']; //导出pdf                          ?></button>
									...
									<?php if ( $typeId !== '5' ): //如果文档类型不为PPT ?>
										<?php if ( $param['op'] == 'edit' && $typeId !== '4' ): ?>
											<div class="mb">
												...
												<button type="button" data-action="selectWord" class="btn fun-btn db mbs"><?php echo $lang['File set']; //插入套红                         ?></button>
												<button type="button" data-action="addPictureFromLocal" class="btn fun-btn db"><?php echo $lang['Insert picture']; //插入图片                         ?></button>
											</div>
										<?php endif; ?>
										<div>
											<div class="mb ovh posr">
												<span class="fun-title"><?php echo $lang['Digital certification']; // 电子认证                        ?></span>
												<span class="fun-divider"></span>
											</div>
											<button data-action="checkSign" data-param='{"key":"<?php echo $param['aid']; ?>"}' type="button" class="btn fun-large-btn db mbs"><?php echo $lang['Verify the signature and seal']; // 验证签名及印章                        ?></button>
											<?php if ( $param['op'] == 'edit' ): ?>
												<button type="button" data-action="fullHandSign" data-param='{"key":"<?php echo $param['aid']; ?>"}' class="btn fun-normal-btn db mbs"><?php echo $lang['Full-screen handwriting signature']; // 全屏手写签名                        ?></button>
												<button type="button" data-action="fullHandDraw" class="btn fun-normal-btn db mbs"><?php echo $lang['Full screen manual drawing']; // 全屏手工绘图                        ?></button>
												<button type="button" data-action="handSign" data-param='{"key":"<?php echo $param['aid']; ?>"}' class="btn fun-normal-btn db mbs"><?php echo $lang['Insert the handwritten signature']; // 插入手写签名                        ?></button>
												<button type="button" data-action="handDraw" class="btn fun-normal-btn db mbs"><?php echo $lang['Insert the manual drawing']; // 插入手工绘图                       ?></button>
												<button type="button" data-action="addSignFromLocal" data-param='{"key":"<?php echo $param['aid']; ?>"}' class="btn fun-normal-btn db mbs"><?php echo $lang['Add local electronic seal']; // 本地电子盖章                       ?></button>
...
<?php endif; ?>
			$(window).on("unload", function() {
				...
				if (OCX.settings.op == 'edit') {
					if (OCX.settings.docOpen) {
						Ui.confirm('是否保存对 ' + OCX.settings.fileName + '的修改？', function() {
							OCX.saveDoc(0);
						});
					}
				}
