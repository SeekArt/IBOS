...
class DefaultController extends BaseController {
	 ...
	 * @return array             按格式处理后的文章列表数据
	 */
	private function handleArticleListDataByCondition( $condition ) {
		$start = util\Env::getRequest( 'start' );
		$length = util\Env::getRequest( 'length' );
		$type = util\Env::getRequest( 'type' );
		$articleList = model\Article::model()->fetchAll( array(
			// 'select'    => 'articleid, subject, author, uptime, addtime, highlightstyle',
			'condition' => $condition,
	...
	private function search() {
		$search = util\Env::getRequest( 'search' );

		if ( isset( $search['keyword'] ) ) {
			$this->condition = ArticleUtil::joinSearchCondition( $search, $this->condition );
		...
		if ( $option == 'default' ) {
			...
			if ( $data['type'] == parent::ARTICLE_TYPE_PICTURE ) {
				...
				foreach ( $params['pictureData'] as $k => $value ) {
					$params['pictureData'][$k]['filepath'] = util\File::imageName( $value['filepath'] );
	...
	private function addOrUpdateArticle( $type, $data, $uid ) {
		...
		//取得发布权限
		$publishScope = util\StringUtil::handleSelectBoxData( $data['publishScope'] );
