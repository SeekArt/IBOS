	...
	<div class="wb-fans-box">
		<div class="wb-per-hd mpanel">
			<ul class="nav nav-skid">
				<li>
					<a href="<?php echo $this->createUrl( 'personal/following', array( 'uid' => $this->getUid() ) ); ?>"><?php if ( $this->getIsMe() ): ?>我<?php else: ?>TA<?php endif; ?>的关注（<?php echo $count['following']; ?>）</a>
				</li>
				<li class="active">
					<a class="active" href="<?php echo $this->createUrl( 'personal/follower', array( 'uid' => $this->getUid() ) ); ?>"><?php if ( $this->getIsMe() ): ?>我<?php else: ?>TA<?php endif; ?>的粉丝（<?php echo $count['follower']; ?>）</a>
				</li>
			</ul>
		</div>
		...
		<?php if ( $count['follower'] > $limit ): ?>
			<a href="javascript:;" class="wb-see-new" data-node-type="loadMoreFollowBtn" data-offset="25" data-action="loadMoreFollow" data-param='{"type": "follower","uid":<?php echo $this->getUid(); ?>}'>
