	...
	<div class="wb-fans-box">
		<div class="wb-per-hd mpanel">
			<ul class="nav nav-skid">
				<li class="active">
					<a class="active" href="<?php echo $this->createUrl( 'personal/following', array( 'uid' => $this->getUid() ) ); ?>"><?php if ( $this->getIsMe() ): ?>我<?php else: ?>TA<?php endif; ?>的关注（<?php echo $count['following']; ?>）</a>
				</li>
				<li>
					<a href="<?php echo $this->createUrl( 'personal/follower', array( 'uid' => $this->getUid() ) ); ?>"><?php if ( $this->getIsMe() ): ?>我<?php else: ?>TA<?php endif; ?>的粉丝（<?php echo $count['follower']; ?>）</a>
				</li>
			</ul>
		</div>
		<div class="wb-fans-list">
						...
						<li>
							<div class="wb-fans-card">
								<div class="wbc-box rdt bdbs">
									<div class="mbs">
										<a data-toggle="usercard" data-param="uid=<?php echo $followState['user']['uid']; ?>" href="<?php echo $followState['user']['space_url']; ?>" class="avatar-circle">
											<img src="<?php echo $followState['user']['avatar_middle']; ?>" alt="<?php echo $followState['user']['realname']; ?>" />
										</a>
									</div>
								...
								<div class="rdb wbc-box2">
									<span class="wb-cb followedboth">
										<?php if ( !$followState['following'] ): ?>
											<a href="javascript:;" class="btn btn-small btn-warning" data-action="follow" data-param='{"fid": <?php echo $followState['user']['uid']; ?>}' data-loading-text="关注中...">
												<i class="om-plus"></i>
												关注
											</a>
										<?php elseif ( $followState['following'] && $followState['follower'] ): ?>
											<a href="javascript:;" class="btn btn-small" data-node-type="unfollowBtn" data-action="unfollow" data-param='{"fid": <?php echo $followState['user']['uid']; ?>}' data-loading-text="取消中...">
												<i class="om-geoc"></i>
												互相关注
											</a>
			...
			<?php else: ?>
				<?php if ( $this->getIsMe() ): ?>我<?php else: ?>TA<?php endif; ?>还没有关注任何人
			<?php endif; ?>
		</div>
		<?php if ( $count['following'] > $limit ): ?>
			<a href="javascript:;" class="wb-see-new" data-node-type="loadMoreFollowingBtn" data-offset="<?php echo WbConst::DEF_LIST_FEED_NUMS; ?>" data-action="loadMoreFollow" data-param='{"type": "following","uid":<?php echo $this->getUid(); ?>}'>
