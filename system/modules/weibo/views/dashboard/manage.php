		...
		<div class="ctb">
			<div class="form-horizontal">
				...
				<div class="btn-group control-group" data-toggle="buttons-radio" id="record">
					<a href="<?php echo $this->createUrl( 'dashboard/manage', array( 'op' => 'list' ) ); ?>" class="btn <?php if ( $op == 'list' ): ?>active<?php endif; ?>"><?php echo $lang['List']; ?></a>
					<a href="<?php echo $this->createUrl( 'dashboard/manage', array( 'op' => 'recycle' ) ); ?>" class="btn <?php if ( $op == 'recycle' ): ?>active<?php endif; ?>"><?php echo $lang['Recycle bin']; ?></a>
				</div>
			</div>
			<div>
				<div class="page-list">
					<div class="page-list-header">
						<div class="btn-toolbar pull-left">
							<button class="btn" type="button" data-param='{"op":"<?php echo $op == 'list' ? 'delFeed' : 'deleteFeed'; ?>"}' data-action="removeWbs"><?php echo $op == 'list' ? $lang['Del weibo'] : $lang['Delete clear']; ?></button>
						</div>
						<div class="search span3 pull-right">
							<input type="text" placeholder="<?php echo $lang['Weibo search tip']; ?>" name="keyword" nofocus id="mn_search" />
					...
					<?php if ( !empty( $list ) ): ?>
						<div class="page-list-mainer">
							<table class="table table-striped">
								<thead>
									<tr>
										<th width="20"><label class="checkbox"><input type="checkbox" data-name="weibo"></label></th>
										<th width="30">ID</th>
								...
								<tbody>
									<?php foreach ( $list as $feed ): ?>
										<tr data-row="<?php echo $feed['feedid']; ?>">
											<td><label class="checkbox"><input type="checkbox" name="weibo" value="<?php echo $feed['feedid']; ?>"></label></td>
											<td><?php echo $feed['feedid']; ?></td>
											<td><div class="wbc-left"><?php echo $feed['body']; ?></div></td>
											<td class="fss"><?php echo $feed['user_info']['realname']; ?></td>
											<td class="fss"><?php echo date( 'n-j H:i', $feed['ctime'] ); ?></td>
											<td>
												<?php if ( $op == 'list' ): ?>
													<a href="javascript:;" class="cbtn o-trash" data-action="removeWb" data-param='{"op":"delFeed","id": <?php echo $feed['feedid']; ?>}'></a>
												<?php else: ?>
													<a href="javascript:;" class="cbtn o-cancel" data-action="recoverWb" data-param='{"op":"feedRecover", "id": <?php echo $feed['feedid']; ?>}'></a>
												<?php endif; ?>
											</td>
										</tr>
									<?php endforeach; ?>
