...
class LocalFile implements FileOperationInterface
{
     ...
     * @param boolean $deleteSrc 是否删除原文件
     * @param boolean $deleteSrc 是否删除原文件
     * @return mixed
     */
    ...
    public function download($attach, $downloadInfo = array())
    {
        ...
        if (file_exists($file)) {
            ...
            $attach['filename'] = Convert::iIconv($attach['filename'], CHARSET, 'gbk');
            if ($content) {
...
//		$this->createFile( $bgPath . $bgMiddle, ' ' );
    ...
    public function setOrgJs($type, $value)
    {
        ...
        return $this->createFile(PATH_ROOT . '/data/org/' . $type . '.js', $value);
    }
    ...
    /**
     * 读取一个网络文件到零时目录并生成临时文件
     * 备注：LocalFile 不需要获取网络文件的本地临时地址，这里只是做兼容处理。
     *
     * @param string $url 要读取的文件 URL
     * @param string $suffix 生成临时文件后缀，为空则自动读取
     * @return string
     */
    public function fetchTemp($url, $suffix = '')
    {
        if (empty($suffix)) {
